...
def boar_brawl(player_score, opponent_score):
    ...
    def score_compial(player_score, opponent_score):
        # 如果玩家选择投掷 0 枚骰子，他们的得分为对手分数的十位数和玩家分数的个位数之间的绝对差的三倍，或者是 1（取两者中的较大者）。
        my_score = player_score % 10
        op_score = (opponent_score % 100) // 10

        s = 3 * abs(my_score - op_score)

        return s

    return max(score_compial(player_score, opponent_score), 1)
...
def sus_update(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    # BEGIN PROBLEM 4
    ps = player_score
    s = simple_update(num_rolls, ps, opponent_score, dice)
    if num_rolls == 0:
        ps = boar_brawl(ps, opponent_score)
    ps = sus_points(s)
    return ps
...
def is_always_roll(strategy, goal=GOAL):
    ...
    st1 = strategy(player_score, opponent_score)

    for player_score in range(goal + 1):
        for opponent_score in range(goal + 1):
            st2 = strategy(player_score, opponent_score)
